...
# On définit le dossier où se trouve le script pour localiser l'image
...
class HaptiMedPDF(FPDF):
    def header(self):
        # Vérification de l'existence du logo avant insertion
...
def clean_text(txt):
    # Gestion des caractères spéciaux pour FPDF (Latin-1)
...
def create_tableau_suivi():
    ...
    cols = ["Code", "Ancienneté (ans)", "Bloc 1", "Bloc 2", "Bloc 3", "Bloc 4", "Commentaires"]
    widths = [15, 30, 18, 18, 18, 18, 70]
...
def create_guide_utilisateur():
    ...
    pdf.multi_cell(0, 6, clean_text("1. Information : Le participant lit la Fiche d'information.\n"
                                    "2. Consentement : Signature du Formulaire.\n"
                                    "3. Anonymisation : Attribution d'un code (ex: P01) et saisie de l'ancienneté."))
    pdf.ln(3)
    ...
    pdf.set_font('Arial', '', 10)
    pdf.multi_cell(0, 5, clean_text("Action : Configurez l'ID. Le sujet suit le tunnel (feedback par épaisseur du trait)."))
    pdf.ln(3)
    ...
    pdf.set_font('Courier', '', 10)
    pdf.cell(0, 6, "python Clean_Data\\process_data.py          # 1. Filtre & IPe/Be", 0, 1, fill=True)
    pdf.cell(0, 6, "python Process_Stat\\analysis_master.py     # 2. Stats APA & Graphs", 0, 1, fill=True)
    pdf.cell(0, 6, "python Process_Stat\\analysis_ml.py         # 3. IA (VP vs FVP)", 0, 1, fill=True)
    pdf.cell(0, 6, "python Paper\\generate_master_report.py     # 4. Rapport HTML", 0, 1, fill=True)
    pdf.ln(3)
...
if __name__ == "__main__":
    ...
    print("Succès ! Les 4 fiches PDF ont été générées.")